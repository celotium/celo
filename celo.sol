...
contract CeloERC20 is Ownable {
    ...
    string public constant standard = "ERC20 CELO";
...
contract Celotium is CeloERC20 {

    // Contract variables and constants
    uint256 constant initialSupply = 0;
    string constant tokenName = "Celotium";
    string constant tokenSymbol = "CELO";

    address public CelotiumAddr = /* TBA */;
    ...
    modifier status() {
        ...
        if (block.timestamp < 1520272800){          // until first date
        ...
        } else if (block.timestamp < 1521136800){   // until second date
          buyPrice = 80000000000000;

        } else if (block.timestamp<1522605600){     // until third date
          buyPrice = 85000000000000;

        } else if (block.timestamp < 1523815200){   //until fourth date
    ...
    function buy() public payable status {
        ...
        CelotiumAddr.transfer(msg.value);
    }
    ...
    function transferBuy(address _to, uint256 _value) internal returns (bool) {
        ...
        held[_to] = block.number;
        balances[CelotiumAddr] = balances[CelotiumAddr].add(_value);
        balances[_to] = balances[_to].add(_value);

        Transfer(this, _to, _value);
        Transfer(this, CelotiumAddr, _value);
        return true;
    }
